...
const Task = function({ id, name, desc, order, subtasks, colId, setBoardsData }) {
    ...
    return (
        <>
            <div ref={setNodeRef} style={style} className="task" onMouseEnter={() => handleDragHandleVis(true)} onMouseLeave={() => handleDragHandleVis(false)} onClick={handleViewTaskModal}>
            ...
            <ViewTask name={name} desc={desc} subtasks={subtasks} colId={colId} taskId={id} setBoardsData={setBoardsData} /> 
            <EditTask name={name} desc={desc} subtasks={subtasks} colId={colId} taskId={id} setBoardsData={setBoardsData} />
        </>
    );
};
