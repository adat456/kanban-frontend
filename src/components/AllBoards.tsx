...
import { UserContext, userInterface, BoardsContext, CurBoardIdContext, boardData } from "../Context";
...
const AllBoards: React.FC<{ setMode: React.Dispatch<React.SetStateAction<string>> }> = function({ setMode }) {
    ...
    const [ curUserStatus, setCurUserStatus ] = useState("");
    ...
    useEffect(() => {
        boardsData?.forEach(board => {
            if (board._id === curBoardId) {
                ...
                if (board.creator === curUser?._id) {
                    setCurUserStatus("Creator");
                } else if (board.contributors) {
                    const contributorProfile = board.contributors.find(contributor => contributor.userId === curUser?._id);
                    if (contributorProfile) setCurUserStatus(contributorProfile.userStatus);
                };
            };
        });
    }, [curBoardId]);
                    ...
                    <CurBoardIdContext.Provider value={{ curBoardId, setCurBoardId }}>
                        ...
                        {!curBoardId ?
                            ...
                            <div className="all-boards">
                                ...
                                <main ref={mainRef}>
                                    <Board setDisplayMsg={setDisplayMsg} curUserStatus={curUserStatus} />
                                </main>
                                <div ref={sidebarBackdropRef} className="sidebar-backdrop" onClick={() => setSidebarVis(false)}></div>
                            </div>
                        }
